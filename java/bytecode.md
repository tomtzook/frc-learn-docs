...
Unfortunately this is not the end of the matter, because _CPU_s cannot actually run _Java Bytecode_. It is an unknown to them. So for all the greatness of having the same _Bytecode_ for all machines, it doesn't just magically work, the proper _Machine Code_ is still necessary, and that's where the _JVM_ steps in.

When a _Java_ binary is executed (like a _jar_ file), it doesn't immediatly just run on the _CPU_ like native program, but rather the _JVM_ is launched with it. The _JVM_ will then be resposible to execute the code. This is done by using a _Just In Time_ (JIT) compiler: when a set of _Bytecode_ is executed for the first time, the _JVM_ will transform the _Bytecode_ into the appropriate _Machine Code_ for the machine that _JVM_ is running on, and then give it to the processor for execution. 
